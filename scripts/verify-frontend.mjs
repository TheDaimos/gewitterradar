...
for(const dest of destinations){
 const wanted=[...payload.keys()];if(dest==='dashboard/dist')wanted.push('app_gewitterradar_pkg.yaml');
...
if(!(await readFile(resolve(root,'dashboard/dist/app_gewitterradar_pkg.yaml'))).equals(pkg))throw Error('Legacy package parity failed');
checks.push(hash(pkg)+'  dashboard/dist/app_gewitterradar_pkg.yaml');
