...
assertExact(sourceBuffer,EXPECTED_SOURCE_BYTES,EXPECTED_SOURCE_SHA,'V4.07.33 main baseline');
assertExact(localeBuffer,EXPECTED_LOCALE_BYTES,EXPECTED_LOCALE_SHA,'V4.07.31 locale baseline');

const source=sourceBuffer.toString('utf8');
const candidate=v407Test34ReviewPolishR2Delta(source);
const candidateBuffer=Buffer.from(candidate,'utf8');

const out=resolve(dir,'gewitterradar-v4.07.34.js');
...
const sums=[
  ...
  `${EXPECTED_SOURCE_SHA}  gewitterradar-v4.07.33.js`,
  `${sha(candidateBuffer)}  gewitterradar-v4.07.34.js`,
  `${EXPECTED_LOCALE_SHA}  locales/about-locales.js`
].join('\n')+'\n';
await writeFile(resolve(dir,'SHA256SUMS.txt'),sums);

console.log(`V4.07.34 bytes: ${candidateBuffer.length}`);
console.log(`V4.07.34 sha256: ${sha(candidateBuffer)}`);
