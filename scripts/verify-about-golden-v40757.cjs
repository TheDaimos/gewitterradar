...
const out = path.join(root, 'artwork/acceptance/premium-controls/golden-v40757');
...
(async () => {
  ...
  try {
    for (const profile of contract.profiles) {
      ...
      console.log(
        `${name}: V4.07.57 About geometry PASS against protected V4.07.56 baseline; delivery pixel diff ${pixels.changedPixels} (${pixels.changedRatio})`,
      );
    }
    ...
    console.log(
      'PASS: V4.07.57 About geometry matches the protected V4.07.56 baseline and same-run delivery pixel contract.',
    );
  } finally {
    await browser.close();
    server.close();
