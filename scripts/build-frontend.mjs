...
export async function expectedPayload(){
 ...
 if(text.includes("const CARD_VERSION = '4.10.02';")){
  ...
  if(!text.includes("const CARD_DISPLAY_VERSION = '4.10.02';")||!text.includes("V4.10.02-MODULAR-DEV-2026-09-21"))throw Error('V4.10.02 markers missing');
 ...
 const inventory=JSON.parse(await readFile(resolve(root,'frontend/assets.json'),'utf8'));
  ...
  const referenced=[...new Set([...assetScanText.matchAll(/new URL\('\.\/assets\/([^'?]+)(?:\?[^']*)?', import.meta.url\)/g)].map(m=>'assets/'+m[1]))].sort();
