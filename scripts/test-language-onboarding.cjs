  ...
  for(const delivery of ['dashboard','integration']){
   ...
   for(const [profile,width,height,hasTouch] of [['desktop',900,850,false],['ipad-landscape',1024,768,true],['ipad-portrait',768,1024,true],['android-portrait',412,915,true],['android-landscape',915,412,true]]){
    ...
    const metrics=await sample.page.evaluate(()=>{
     const dialog=window.card._languageOnboardingDialog,intro=dialog.querySelector('.language-intro'),options=dialog.querySelector('.language-options'),title=dialog.querySelector('h2'),style=getComputedStyle(intro),optionStyle=getComputedStyle(options),choice=dialog.querySelector('label span'),legends=[...dialog.querySelectorAll('legend')];
    ...
    assert.ok(metrics.fontSize>=metrics.choiceFontSize-.1,profile+' intro smaller than choices');
    assert.ok(metrics.legendFontSizes.every(size=>size>=metrics.choiceFontSize-.1),profile+' legend smaller than choices');
