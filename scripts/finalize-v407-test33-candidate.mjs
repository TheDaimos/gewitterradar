...
const candidate=v407Test33ReviewPolishDelta(source);
const candidateBuffer=Buffer.from(candidate,'utf8');

const out=resolve(dir,'gewitterradar-v4.07.33.js');
...
const sums=[
  ...
  `${sha(candidateBuffer)}  gewitterradar-v4.07.33.js`,
  `${EXPECTED_LOCALE_SHA}  locales/about-locales.js`
].join('\n')+'\n';
await writeFile(resolve(dir,'SHA256SUMS.txt'),sums);

console.log(`V4.07.33 bytes: ${candidateBuffer.length}`);
console.log(`V4.07.33 sha256: ${sha(candidateBuffer)}`);
