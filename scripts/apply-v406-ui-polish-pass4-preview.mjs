...
if (!inputs.every((value) => value === inputs[0])) {
  throw new Error('Frontend deliveries differ before UI-polish pass4 preview; aborting without writes.');
}
if (!inputs[0].includes("V4.06 pass3: suppress WebKit's reopen focus frame")) {
  throw new Error('V4.06 UI-polish pass3 must be applied before pass4.');
}
if (inputs[0].includes('V4.06 pass4: keep programmatic dialog focus invisible')) {
  console.log('V4.06 UI-polish pass4 preview is already applied; no files changed.');
  process.exit(0);
}

const output = v406UiPolishPass4Delta(inputs[0]);
for (const relative of targets) await writeFile(resolve(root, relative), output, 'utf8');

console.log('Applied V4.06 UI-polish pass4 byte-identically to all three frontend deliveries.');
console.log('Pass4 changes: suppress touch-tablet dialog focus outline, vertically center Welcome footer controls, reuse the exact Settings signature asset in the Welcome footer.');
