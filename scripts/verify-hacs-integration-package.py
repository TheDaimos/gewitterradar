...
def verify(stage_root:Path)->dict[str,str]:
 ...
 files={path.relative_to(target).as_posix():path for path in target.rglob("*") if path.is_file()}
 missing=sorted(REQUIRED-files.keys())
 if missing:raise RuntimeError(f"Missing integration files: {missing}")
...
def main():
 ...
 if args.stage:hashes=verify(args.stage.resolve())
 else:
  with tempfile.TemporaryDirectory(prefix="gewitterradar-hacs-") as temporary:hashes=verify(Path(temporary))
