...
INPUT_SHA256 = "16bee52c4fecddad624497ffc99fb09d9bb82a15544ae5d68c72fe6937798013"
...
def replace_once(card: str, label: str, needle: str, replacement: str) -> str:
    ...
    if count != 1:
        raise RuntimeError(f"V4.08.09 {label}: expected exactly one anchor, found {count}")
...
if input_hash != INPUT_SHA256:
    raise RuntimeError(f"V4.08.09 builder refuses unknown V4.08.08 input: {input_hash}")

card = replace_once(card, "header", "/* Gewitterradar Card V4.08.08 TEST", "/* Gewitterradar Card V4.08.09 TEST")
card = replace_once(card, "CARD_VERSION", "const CARD_VERSION = '4.08.08';", "const CARD_VERSION = '4.08.09';")
card = replace_once(card, "CARD_DISPLAY_VERSION", "const CARD_DISPLAY_VERSION = '4.08.08';", "const CARD_DISPLAY_VERSION = '4.08.09';")
card = replace_once(card, "build", "const GEWITTERRADAR_BUILD = 'V4.08.08-SESSION-AB-COMPARE-2026-09-17';", "const GEWITTERRADAR_BUILD = 'V4.08.09-SESSION-STATIC-GOLD-2026-09-17';")
card = replace_once(card, "weather lab version", "version:'4.08.08',seed:40802", "version:'4.08.09',seed:40802")
card = replace_once(card, "weather lab title", "Wetter-Labor · V4.08.08", "Wetter-Labor · V4.08.09")

old_row = '''                  <div class="settings-row settings-radius-main-toggle-row settings-cluster-session-row settings-cluster-session-ab-row">
...
new_row = '''                  <div class="settings-row settings-radius-main-toggle-row settings-cluster-session-row">
                    ...
                    <div class="settings-cluster-session-selector settings-cluster-session-static-gold" id="settings-cluster-jump-selector" data-mode="finite" role="group" aria-label="Sitzungszeit des Cluster-Sprungs">
                      <label class="settings-cluster-session-mode settings-cluster-session-finite" id="settings-cluster-jump-finite"><input id="settings-cluster-jump-seconds" type="number" min="5" max="3600" step="1" inputmode="numeric" value="10" aria-label="Sitzungszeit des Cluster-Sprungs in Sekunden"><span class="settings-cluster-session-unit">s</span></label>
...
card = replace_once(card, "final B markup", old_row, new_row)

css_pattern = re.compile(r'''          /\* V4\.08\.08 – direkter A/B-Vergleich der Sitzungszeit\..*?          @media\(max-width:520px\) \{\n            \.settings-cluster-session-ab-row \{ align-items:flex-start; \}\n            \.settings-cluster-session-ab-stack \{ margin-left:8px; \}\n          \}\n''', re.S)
new_css = '''          /* V4.08.09 – gewählte Variante B: statische Goldbetonung, keine Animation. */
          .settings-cluster-session-static-gold .settings-cluster-session-mode { position:relative;overflow:hidden; }
          ...
          .settings-cluster-session-static-gold.is-infinite .settings-cluster-session-finite { opacity:.44; }
          .settings-cluster-session-static-gold:not(.is-infinite) .settings-cluster-session-infinite { opacity:.72; }
'''
card, css_count = css_pattern.subn(new_css, card, count=1)
if css_count != 1:
    raise RuntimeError(f"V4.08.09 A/B CSS removal: expected 1 block, found {css_count}")

old_lookup = '''      const settingsClusterJumpSelectors = ['a','b'].map(v => this.shadow.getElementById(`settings-cluster-jump-selector-${v}`)).filter(Boolean);
...
new_lookup = '''      const settingsClusterJumpSelector = this.shadow.getElementById('settings-cluster-jump-selector');
...
card = replace_once(card, "final B lookup", old_lookup, new_lookup)

old_handlers = '''      const applyClusterJumpSeconds = (input) => {
        if (!input) return;
...
new_handlers = '''      const applyClusterJumpSeconds = () => {
...
card = replace_once(card, "final B handlers", old_handlers, new_handlers)

old_sync = '''      const settingsClusterJumpSelectors = ['a','b'].map(v => $(`settings-cluster-jump-selector-${v}`)).filter(Boolean);
...
card = replace_once(card, "final B sync", old_sync, new_sync)

required = [
    "const CARD_VERSION = '4.08.09';",
    "V4.08.09-SESSION-STATIC-GOLD-2026-09-17",
    "settings-cluster-session-static-gold",
    "linear-gradient(110deg,rgba(116,78,15,.30),rgba(223,173,66,.30) 45%,rgba(255,218,128,.13) 70%,rgba(14,29,38,.20))",
    "animation:none;",
...
for marker in required:
    if marker not in card:
        raise RuntimeError(f"V4.08.09 missing required marker: {marker}")

for forbidden in (
    "settings-cluster-session-variant-a",
    "settings-cluster-session-variant-b",
    "settings-cluster-session-ab-stack",
    "v40808-gold-ltr",
    "v40808-gold-rtl",
    ...
    if forbidden in card:
        raise RuntimeError(f"V4.08.09 still contains retired A/B marker: {forbidden}")

for path in (SOURCE, INTEGRATION, DASHBOARD):
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(card, encoding="utf-8")

print("V4.08.09 frontend SHA256", sha256_text(card))
print("V4.08.09 static gold session control applied")
