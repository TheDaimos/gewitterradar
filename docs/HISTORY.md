...
Die reale Sprachabnahme der Modulansicht zeigte anschließend noch einen separaten Altbestand: Feldbezeichnungen, Gruppen und Status waren bereits lokalisiert, die aus den Modulmetadaten übernommenen **Modulnamen und Funktionslisten** blieben jedoch deutsch. V4.10.02 besitzt deshalb nun einen vollständigen 19-Sprachen-Metadatenvertrag für **alle 22 Module**. In der Ansicht werden Anzeigename und Funktionsumfang aus dem aktiven Sprachsatz erzeugt; technische Modul-IDs wie `diagnostics.module-view` und Dateipfade bleiben bewusst unverändert. Für Griechisch prüft der Browservertrag zusätzlich alle 22 gerenderten Modulnamen und Funktionslisten auf tatsächlich griechischen Inhalt und verbietet die konkret beobachteten deutschen Resttexte. Die Statuszeile des Modulfensters wurde außerdem in drei klar getrennte Segmente mit expliziten Trennpunkten überführt, damit Version, Ladezahl und Versionskonsistenz nicht mehr aneinanderkleben.

Ein weiterer realer UI-Test zeigte beim benutzerdefinierten **Standardansicht**-Dropdown einen Lifecycle-Restfehler: Wurde das Dropdown geöffnet und anschließend durch einen Klick auf den Hintergrund der Einstellungsdialog geschlossen, konnte das aus dem Dialoglayout herausgelöste Dropdown sichtbar über der Karte stehen bleiben. Der Kartenansichtsbaustein stellt deshalb nun eine zentrale Schließroutine für dieses Dropdown bereit; der Einstellungs-Lifecycle ruft sie beim Öffnen/Schließen, beim Hintergrundklick und beim Wechsel des Akkordeonabschnitts auf. Der Browservertrag reproduziert sowohl den Hintergrundklick als auch den Abschnittswechsel und verlangt jeweils ein geschlossenes Dropdown mit zurückgesetztem `aria-expanded`.

Während der abschließenden M13-Geräteabnahme wurden zwei weitere reale Regressionen gefunden und korrigiert: Beim Absenken des **Gewitterradius** unter einen noch höheren gespeicherten **Gefahrenradius** hatte die visuelle Vorschau den inneren Regler bereits reduziert, der Speichervorgang wertete jedoch genau diesen Vorschauwert statt des noch höheren Home-Assistant-Istwerts aus. Dadurch konnte der notwendige Gefahrenradius-Schreibvorgang ausfallen und die native Invariantenprüfung den Gewitterradius ablehnen. Die Kaskade verwendet nun den tatsächlich gespeicherten Istwert und reduziert Gefahr vor Gewitter zuverlässig. Zusätzlich wurde der rechteckige Fokus-/Darstellungsrahmen um das runde Premium-Schließen-X der Kompassauswahl auf iPad und Android entfernt; der Tastaturfokus bleibt über einen dezenten Bildglanz sichtbar.

Im selben M13-Abschlusszug wurde die Cluster-Navigation für den Kartenfokus im Vollbild ergänzt. Neben den bestehenden Kompass- und Medaillon-Schaltern sitzt nun das vorhandene **Hi-Res-Infinity-Symbol** als dritter Instrumentschalter. Es blendet eine eigene Cluster-Jump-Pille ein oder aus. Die Pille spiegelt ausschließlich den bestehenden Cluster-Navigationszustand, erzeugt also keine zweite Navigationslogik, startet links neben dem 3D-Layer-Schalter, lässt sich per Maus und Touch frei verschieben und speichert Position sowie Sichtbarkeit lokal pro Browser. Auch der bestehende Sitzungs-/Unendlich-Zustand wird direkt aus derselben Cluster-Navigation übernommen.

Vor der Erweiterung um weitere Medaillonvarianten wurde außerdem die Diagnose auf die neuen Instrument-Pop-ups ausgedehnt. Ist die globale Diagnose aktiv und ihre Darstellung sichtbar, zeigen Kompass- und Medaillon-Auswahl nun ihre eigenen lokalen Messachsen, Mittelpunkt- und Begrenzungsmarken sowie einen kompakten Live-Messwertblock direkt im Dialog-Top-Layer. Damit bleiben die Messhilfen auch dort sichtbar, wo ein außerhalb des nativen `<dialog>` liegendes Overlay nicht zuverlässig über dem Popup dargestellt werden könnte. Beim Kompass werden zusätzlich Pivot und Chivron-/Zählersymmetrie gemessen; beim Medaillon werden Apertur, Soll-/Ist-Pfeilzentrum, Pfeilgröße und die goldene Navigationsreihe geprüft.

Die Medaillon-Diagnose ist dabei jetzt **designfähig**. Jeder Eintrag in `MEDALLION_DESIGNS` kann ein eigenes `diagnosticProfile` mit Quellgeometrie, Apertur, Motivzentrum, Pfeilzentrum und Sollgröße tragen. Die Kalibrierung verwendet immer das aktuell ausgewählte Medaillon statt eines fest verdrahteten ersten Eintrags. Das bestehende `trend_01` übernimmt seine bereits abgenommenen Messwerte unverändert und bildet damit die Referenz für die nachfolgenden Medaillonvarianten. Picker-Messwerte werden zusätzlich in den Diagnose-Snapshot aufgenommen.

Bei der ersten realen Nutzung dieser Erweiterung wurden zwei Bedienungslücken sichtbar. Erstens sprang der Medaillon-Diagnosezustand nach LEER/PFEIL/TREND/FREEZE bei einem normalen Renderlauf wieder auf NORMAL zurück, weil die abgeschaltete Kalibrieransicht ihre Aufräumroutine bei jeder Synchronisierung ausführte. Diese Kopplung ist entfernt: Solange der globale Diagnosemodus läuft, bleibt der gewählte Medaillon-Testzustand stabil; auf NORMAL wird erst beim tatsächlichen Verlassen der Diagnose zurückgeschaltet. Zweitens lag die große Diagnosekonsole außerhalb des nativen Popup-Top-Layers und war dort nicht bedienbar. Deshalb besitzen Kompass- und Medaillon-Picker nun ihre eigene kompakte Werkzeugleiste innerhalb des Dialogs. Das Medaillon bietet dort dieselben Zustände sowie Pfeil-, Animations-, Freeze- und Winkelsteuerungen; beide Picker können ihren strukturierten Messdatensatz direkt in die Zwischenablage kopieren oder als JSON beziehungsweise CSV exportieren. Auf niedrigen Querformat-Viewports wird der Dialog bei Bedarf intern scrollbar, damit Instrument, Diagnose und Export erreichbar bleiben.
Bei der anschließenden realen R5-Abnahme wurden noch drei technische Unterschiede zwischen internem Diagnosezustand und tatsächlicher Darstellung sichtbar: Die TREND-Animation meldete sich als aktiv, der Pfeil blieb jedoch wegen einer statischen `!important`-Transformation stehen; die Winkelbuttons verwendeten die um etwa 45° vororientierte Pfeilgrafik als Nullpunkt; und die lokalen SVG-Messhilfen waren trotz aktivem Zustand im Top-Layer nicht sichtbar. R6 führt deshalb eine explizite absolute Winkelkonvention (**0° Nord, 90° Ost, 180° Süd, 270° West, im Uhrzeigersinn**) mit −45° Assetkompensation ein und trennt statische Transformation von echter Keyframe-Animation.

Die Top-Layer-Diagnose wurde gleichzeitig auf alle neuen Ansichten erweitert. Kompass- und Medaillon-Picker besitzen eigene eindeutig bezeichnete Raster `KP-A1…KP-J10` und `MP-A1…MP-J10`; der Vollbildmodus besitzt `FS-A1…FS-J10`. Im Vollbild werden Diagnose-Overlay und große Diagnosekonsole in den nativen Vollbilddialog verschoben. Öffnet sich darüber ein Instrument-Picker, folgt die Diagnosekonsole dem obersten Dialog und kehrt beim Schließen wieder in den Vollbilddialog zurück. Die Browserregression prüft nun zusätzlich tatsächliches SVG-Markup, sichtbare Transformänderung während TREND sowie die Top-Layer-Hostwechsel.
...
Die anschließende reale R7-Abnahme über DRA/HA bestätigte den letzten Teardown-Fix: Wird die Diagnose bei geöffnetem Instrument-Picker beendet, verschwinden Raster und Messhilfen sofort, während der Picker geöffnet bleibt. Damit ist der Diagnosekorrekturblock R5–R7 abgeschlossen; die zuvor bestätigten Funktionen für sichtbare TREND-Animation, absolute Winkel, KP-/MP-Raster und erreichbare Diagnosekonsole bleiben funktionsfähig. Weitere Medaillon-Designs können nun auf dem abgenommenen Diagnosefundament fortgesetzt werden.


Beim formalen Abschlussaudit der V4.10.02-Modularisierung wurden die ursprünglichen offenen Checkboxen gegen die tatsächlich später ausgeführten DRA-, Browser-, Geräte- und Diagnoseabnahmen zurückgeprüft. Dadurch konnten zahlreiche nur noch historisch offene Punkte geschlossen werden. Gleichzeitig fand der Audit zwei echte Metadatenreste: Das Modulmanifest erwartete bereits eine neuere eigene Modulversion, registrierte sich selbst aber noch als 1.2.10, und der ausgelagerte Basiskontext führte intern noch die R5-Buildkennung. Der reine Konsistenzstand R8 vereinheitlicht diese Identitäten und ergänzt einen fail-closed Vertrag, der alle 22 Soll-Modulversionen gegen ihre Selbstregistrierung prüft. Der R8-Kandidat `8ee2b6fbc30213ead936e622cc886afd995dcff6` bestand anschließend alle fünf CI-Gates und wurde exakt nach `deploy/dev` promoviert. Formal offen bleiben damit nur noch die reale R8-DRA-Laufzeitbestätigung mit 22/22 konsistenten Modulen sowie die Cluster-Jump-/Infinity-Geräteabnahme; danach folgen die bereits vorbereiteten weiteren Medaillon-Designs.


Nach der realen R9-Abnahme des neuen Abweichungsdialogs zeigte dessen Export, dass die verbliebenen Modulabweichungen nicht aus vier unabhängigen Fehlern bestanden, sondern aus gemischten internen ES-Modul-URLs mit alten und aktuellen Cachekennungen. R10 vereinheitlichte sämtliche statischen internen Modulimporte auf `41002r10` und ergänzte einen Regressionstest gegen abweichende Importkennungen. Der finale Kandidat `4f22f4be5841e47993226928405cc65cdd70e201` bestand alle fünf CI-Gates und wurde über DRA real mit **22/22 geladenen Modulen**, **konsistentem Versionssatz** und Modulsatz `CEA6-1ECF` abgenommen. Damit ist der gesamte Modul-/Abweichungsblock abgeschlossen; im Modularisierungs-Schlachtplan bleibt nur noch die reale Cluster-Jump-/Infinity-Geräteabnahme offen.
